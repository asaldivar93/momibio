...
#Load Data
data = np.loadtxt("CIR_B3_020420_0.txt", delimiter ='\t', skiprows = 11)
data2 = np.loadtxt('CIR_B3_020420_1.txt', delimiter ='\t', skiprows = 0)
data3 = np.loadtxt('CIR_B3_020420_2.txt', delimiter ='\t', skiprows = 0)
data4 = np.loadtxt('CIR_B3_020420_3.txt', delimiter ='\t', skiprows = 0)

blank = np.loadtxt('blank.txt', delimiter ='\t', skiprows = 1)
blank = np.mean(blank[:,1])
#Concat Files
...
window_size, poly_order = len(Time[Time<6]), 3
OD = np.log10(blank/Light_intensity[:]) #Optical Density
...
np.savetxt('resultados_0401_2.txt', save, delimiter = '\t')    
...
#plot.setp(ax_OD.get_yticklabels(), visible = False)
